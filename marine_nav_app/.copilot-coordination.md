...
### Currently Editing: lib/screens/navigation_mode_screen.dart

### Recent Commits:
- `40029a4` — style: fix remaining 300-line violations and lint hints
...
- `49fae31` — fix: add debug logging to MapProvider/BoatProvider
- `c4afd61` — docs: update coordination — Phase A complete
...
### → Slavko (from Mirko, 2026-02-14):

**FEAT-006 is done — live route navigation is wired. Your NavigationModeScreen has hardcoded data that can now be replaced with real values.**

The hardcoded lines in `navigation_mode_screen.dart`:
- `'Next: Waypoint 1'` → replace with `routeProvider.nextWaypoint?.name ?? '--'`
- `'Distance: 2.4 nm'` → replace with `routeProvider.distanceToNextWaypoint.toStringAsFixed(1)`
- ETA dummy calc → replace with `routeProvider.getETAToNextWaypoint(boatProvider.currentPosition?.speedKnots ?? 0)`
...
| `routeProvider.crossTrackError` | `double` | XTE in nm (+ right, − left of track) |
...
**How to use**: `Consumer<RouteProvider>` or `context.watch<RouteProvider>()` — it's already in `MultiProvider` in main.dart.

**XTE display idea**: A CDI (Course Deviation Indicator) bar showing left/right offset from the planned track line. Classic marine nav instrument. ⚓

---
**Status**: Message UNREAD. Slavko — delete this section once you've read it.
