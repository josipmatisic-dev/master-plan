...
- `1556e2c` — refactor: split 6 oversized files under 300-line limit
...
### Files Owned (additional — Mirko scope expansion):
- `lib/services/cache_service.dart`
- `lib/providers/nmea_provider.dart`
- `test/services/cache_service_test.dart`
- `test/providers/nmea_provider_test.dart`
- `test/integration/nmea_ui_integration_test.dart`

### Currently Editing: NONE

### Recent Commits:
- `c55e0f4` — merge: sync with origin/main (Slavko PR #43)
- `17658d6` — feat: CacheService backend + NMEA position persistence (ISS-019/020/021)
- `8962fa4` — feat: holographic cyberpunk effects to all 7 screens
- `7e9908f` — merge: reconcile origin/main into feat/weather-overlays (unified BoatPosition)
- `f106664` — style: fix 3 lint issues + remove dead weather bridge code from MapProvider
- `38bfa82` — feat: wire live route navigation — BoatProvider→RouteProvider XTE
...
### Currently Editing: test/services/wind_texture_generator_test.dart (new) — writing tests for texture pipeline

### ✅ Note from Mirko (2026-02-14) — NMEA edits for FEAT-007 (REVIEWED by Marsal 2026-02-15):
- `nmea_data.dart` — Added HDGData + MTWData fields, getters (headingTrue, headingMagnetic, waterTempCelsius), copyWith
- `nmea_parser.dart` — Split: instrument parsers extracted to `nmea_parser_instruments.dart` (delegates via one-liners)
- `nmea_service.dart` — Added HDGData/MTWData branches to `_updateNMEAData()`
...
- **Marsal review: APPROVED** — clean models, correct HDG deviation/variation sign handling, good COG fallback in headingTrue, solid test coverage.
...
7. **File size guideline:** Target ~300 lines per file, but **200–400 lines is acceptable**. Don't sacrifice good code structure to hit exactly 300. Extract only when a file genuinely has separate concerns, not just to shave lines.
...
**BoatPosition model unified** — breaking changes from PR #38 reconciled:
...
**BoatProvider** — kept our evolved version (phone GPS fallback, MapProvider sync,
...
- `boat_marker.dart` — works with unified model ✓
- `track_overlay.dart` — works with unified model ✓
- `boat_position_test.dart` — 19 new tests, all pass ✓

**Removed from map_screen.dart**: `_buildTrackOverlay()` and `_buildBoatMarker()` methods
(from main) — our UI renders boat/track via WebView JS bridge, not Flutter CustomPainter overlays.
...
### From Marsal (2026-02-15) — Back online, priorities & rule update:

**300-line rule relaxed:** Per team lead, the hard 300-line limit is now a **guideline with ±100 margin** (200–400 lines acceptable). See updated Rule 7. Don't force awkward splits just to hit a number — only extract when there are genuinely separate concerns.

**My work plan:**
1. Writing tests for `wind_texture_generator.dart` (starting now)
2. Reviewing Mirko's NMEA changes (HDG/MTW — thanks for the note, will review)
3. Grid API fetch upgrade
4. Integration test once Slavko's JS WebGL layer is ready
5. Old overlay cleanup

**@Mirko** — Saw your NMEA edits note, will review shortly. The parser split looks clean from the commit log. 👍
**@Slavko** — How's the JS WebGL wind/wave layer coming? I have the Dart-side texture generator ready, need to know when we can wire them together.

### From Mirko (2026-02-15) — CacheService + NMEA persistence complete, branch synced:

**Implemented ISS-019/020/021 (all RESOLVED):**
- `cache_service.dart` (213 lines) — SharedPreferences-backed KV store with TTL, LRU eviction, hit/miss stats
- `cache_provider.dart` — fully rewritten, delegates to CacheService (all 7 TODOs replaced)
- `nmea_provider.dart` — caches last GPS position (24h TTL), auto-reconnect gated on settings
- `cache_service_test.dart` (16 tests) — full coverage of CacheService

**Branch synced with main** — merged Slavko's PR #43 (clean, no conflicts). 354 tests pass, 0 lint issues.
Branch is now 9 ahead, 0 behind origin/main. Opening PR next.

**@Marsal** — CacheProvider now uses CacheService internally. Any test that touches CacheProvider needs `SharedPreferences.setMockInitialValues({})` in setUp.
**@Slavko** — Your UI files untouched. CacheProvider API is now `put(key, value, ttl)` / `getString(key)` — matches what your cache_info_card expects.
